...
const safeExecuteJavaScript = async (view, script, description) => {
  ...
  const viewNum =
    view.webContents.id === appContext.view1.webContents.id ? "1" : "2"; // Determine view number based on webContents ID
  ...
  try {
    ...
    const p2LoadPromise = new Promise((resolve, reject) => {
      ...
      successHandler = () => {
        ...
        console.log("P2 loaded lobby URL.");
        if (failHandler)
    ...
    try {
      ...
      if (
        ...
        console.log("Reloading V1 after error.");
        view1.webContents
          .loadURL("https://karabast.net")
          .catch((err) => console.error("Failed reload V1 after error:", err));
      }
      ...
      console.error("Failed reset view state after error:", resetError);
    }
  } finally {
