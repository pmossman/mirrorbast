...
if (window.api) {
  const {
    readClipboard, fetchMetadata, switchPlayer, resetApp, autoSetup,
    sidebarStateChanged,
    // *** NEW: Import visibility control ***
    setGameViewsVisibility,
    onLobbySuccess, onResetSuccess, onResetError, onAutoSetupError,
    onTriggerSwitch, onSetSidebarCollapsed,
  ...
  async function handleAddDeck() { // Used in Manage Decks view
    ...
    try {
      ...
      if (metadata && metadata.name && metadata.name !== 'Fetch Error' && metadata.name !== 'Invalid Metadata') {
         ...
         renderDeckList(); populateSelects(); deckInputElement.value = '';
  ...
  function populateSelects() {
    ...
    [p1Select, p2Select].forEach((select, index) => {
      ...
      savedDecks.forEach(deck => {
          ...
          opt.title = `${deck.name || 'Unnamed Deck'} (${deck.author || 'Unknown'})`;
 ...
 /** Handles adding/selecting a deck from quick-add inputs */
  async function handleAddAndSelectDeck(urlInputEl, targetSelectEl, addButtonEl, playerNum) {
      ...
      addButtonEl.textContent = '...'; addButtonEl.disabled = true;
      try {
          ...
          if (existingDeck) {
              ...
              const metadata = await fetchMetadata(url);
              if (metadata && metadata.name && metadata.name !== 'Fetch Error' && metadata.name !== 'Invalid Metadata') {
                  savedDecks.push({ url, ...metadata }); saveDecks(); populateSelects();
  ...
  function handleAutoSetup() {
    ...
    switchBtnFooter.disabled = true; resetBtn.disabled = true; manageDecksBtn.disabled = true;
  ...
  function handleReset() {
      ...
      if (confirm('Reset both player views to Karabast home?')) {
          ...
          autoSetupBtn.disabled = true; switchBtnFooter.disabled = true; manageDecksBtn.disabled = true;
          p1AddSelectBtn.disabled = true; p2AddSelectBtn.disabled = true;
          resetApp();
      }
  }
  ...
  function onGameReady() {
    ...
    switchBtnFooter.disabled = false; resetBtn.disabled = false; resetBtn.textContent = 'Reset'; manageDecksBtn.disabled = false;
  ...
  function onSetupError(errorMessage) {
   ...
   function onResetComplete() {
       ...
       resetBtn.textContent = 'Reset'; resetBtn.disabled = false;
       autoSetupBtn.disabled = false; switchBtnFooter.disabled = false; manageDecksBtn.disabled = false;
       p1AddSelectBtn.disabled = false; p2AddSelectBtn.disabled = false;
       saveLastDeckSelection(1, ''); saveLastDeckSelection(2, '');
       populateSelects();
   ...
   function handleSetSidebarCollapsed(collapsed) {
       console.log(`Renderer: Received request to set sidebar collapsed: ${collapsed}`);
    ...
    function handleCollapseRequest() {
        ...
        if (!bodyElement.classList.contains('deck-view-active')) {
            console.log("Renderer: Received request to collapse sidebar after auto-setup.");
  ...
  function setupEventListeners() {
    ...
    p1AddSelectBtn.addEventListener('click', () => handleAddAndSelectDeck(p1UrlInput, p1Select, p1AddSelectBtn, 1));
    p2AddSelectBtn.addEventListener('click', () => handleAddAndSelectDeck(p2UrlInput, p2Select, p2AddSelectBtn, 2));
    p1Select.addEventListener('change', () => saveLastDeckSelection(1, p1Select.value));
    p2Select.addEventListener('change', () => saveLastDeckSelection(2, p2Select.value));
